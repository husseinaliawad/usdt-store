...
<head>
    ...
    <title>USDT STORE - لوحة التحكم</title>
    <style>
        :root {
            --bg: #0a0b0d;
            --surface: #121417;
        ...
        * { box-sizing: border-box; }
        ...
        body {
            margin: 0;
            color: var(--text);
            background: linear-gradient(180deg, #0f1115 0, var(--bg) 420px);
        }
        ...
        .mark {
            ...
            color: #111;
            font-weight: 900;
            background: linear-gradient(135deg, #f4d27b, var(--gold));
        }
        ...
        .notice {
            margin: 0 0 18px;
            padding: 12px 14px;
            border: 1px solid rgba(64, 199, 121, .26);
            border-radius: 8px;
        ...
        .btn {
            ...
            color: #101114;
            font-weight: 800;
            background: linear-gradient(90deg, #f3d17d, var(--gold));
        }
        .btn.secondary {
            color: var(--text);
            border: 1px solid var(--line);
        ...
        .btn.danger {
            color: #260909;
            background: linear-gradient(90deg, #ff9a9a, var(--red));
        }
        ...
        .kpi, .panel, .module {
            border: 1px solid var(--line);
            border-radius: 8px;
            background: linear-gradient(180deg, rgba(25, 29, 34, .98), rgba(16, 18, 21, .98));
        ...
        .bar {
            ...
            background: linear-gradient(180deg, #f0cc75, #6cbf91);
        }
        .bar-label { color: var(--muted); font-size: 11px; text-align: center; white-space: nowrap; }
        .bar-value { color: #dfe7ee; font-size: 11px; text-align: center; white-space: nowrap; }
        ...
        .compact-form {
            display: grid;
            grid-template-columns: 132px 140px minmax(150px, 1fr) auto;
            gap: 8px;
            align-items: center;
        }
        .form-grid {
        ...
        .module { padding: 15px; }
        .module ul { margin: 13px 0 0; padding: 0; list-style: none; display: grid; gap: 8px; }
        .module li {
            display: flex;
            justify-content: space-between;
            gap: 10px;
...
@php
    ...
    $maxMonthly = max((float) $monthlyMovement->max('amount'), 1);
@endphp
...
<div class="shell">
    <aside class="sidebar">
        ...
        <nav class="nav">
            <div class="nav-group">
                <div class="nav-title">المراقبة</div>
                <a href="#overview"><span class="nav-icon">▦</span> الصفحة الرئيسية</a>
                <a href="#reports"><span class="nav-icon">↟</span> التقارير</a>
                <a href="#audit"><span class="nav-icon">⌁</span> سجل العمليات</a>
            </div>
            <div class="nav-group">
                <div class="nav-title">الإدارة</div>
                <a href="#users"><span class="nav-icon">◌</span> المستخدمون و KYC</a>
                <a href="#wallets"><span class="nav-icon">▤</span> المحافظ</a>
                <a href="#transfers"><span class="nav-icon">↔</span> التحويلات</a>
                <a href="#currencies"><span class="nav-icon">$</span> العملات والرسوم</a>
                <a href="#orders"><span class="nav-icon">□</span> الطلبات</a>
            </div>
            <div class="nav-group">
                <div class="nav-title">النظام</div>
                <a href="#notifications"><span class="nav-icon">◉</span> الإشعارات</a>
                <a href="#roles"><span class="nav-icon">◇</span> الصلاحيات</a>
                <a href="#security"><span class="nav-icon">⎋</span> الأمان</a>
                <a href="#settings"><span class="nav-icon">⚙</span> الإعدادات</a>
                <a href="#support"><span class="nav-icon">?</span> الدعم الفني</a>
    ...
    <main class="content">
        <section class="topbar" id="overview">
            <div>
                <h1>لوحة التحكم</h1>
                <div class="subtitle">إدارة الرصيد، المستخدمين، التحويلات، المحافظ، الرسوم، التقارير، السجل، والصلاحيات من شاشة واحدة.</div>
            </div>
        ...
        <form class="toolbar" method="get" action="{{ route('control.dashboard', [], false) }}">
        ...
        <section class="kpi-grid">
            ...
            <div class="kpi"><span>مرفوضة أو ملغاة</span><strong>{{ number_format(($statusCounts['rejected'] ?? 0) + ($statusCounts['failed'] ?? 0)) }}</strong><small>تحتاج متابعة سبب الرفض</small></div>
        </section>

        <section class="two-col">
            <div class="panel">
                <div class="panel-head"><div><h2>رسم بياني للحركة اليومية</h2><small>حجم العمليات خلال آخر 7 أيام</small></div></div>
            ...
            <div class="panel">
                <div class="panel-head"><div><h2>رسم بياني للحركة الشهرية</h2><small>آخر 6 أشهر حسب حجم التحويلات</small></div></div>
                <div class="panel-body">
        ...
        <section class="panel" id="transfers">
            <div class="panel-head">
                <div><h2>التحويلات</h2><small>جميع التحويلات، قيد المراجعة، قيد التنفيذ، مكتملة، مرفوضة، إلغاء وتعديل الحالة وإثبات الدفع</small></div>
                <span class="badge">{{ $transactions->count() }} عملية معروضة</span>
            </div>
        ...
        <section class="panel" id="users">
            <div class="panel-head"><div><h2>إدارة المستخدمين</h2><small>عرض المستخدمين، البحث والتصفية، تعديل الصلاحية، تفعيل أو إيقاف الحساب، KYC، المحافظ وسجل النشاط</small></div></div>
            <div class="table-wrap">
                <table>
        ...
        <section class="panel">
            <div class="panel-head"><div><h2>توثيق الهوية KYC</h2><small>طلبات التوثيق المرتبطة بالمستخدمين</small></div></div>
            <div class="table-wrap">
                <table>
                    ...
                    @forelse ($kycVerifications as $kyc)
                        <tr>
                            ...
                            <td>{{ $kyc->full_name }}</td>
                            <td>{{ $kyc->phone }}</td>
                            <td><span class="badge {{ $statusClass[$kyc->status] ?? 'blue' }}">{{ $statusLabels[$kyc->status] ?? $kyc->status }}</span></td>
                            <td>{{ $kyc->admin_note ?: '-' }}</td>
                            <td>{{ $kyc->updated_at?->format('Y-m-d H:i') }}</td>
                        </tr>
                    @empty
        ...
        <section class="panel" id="wallets">
            <div class="panel-head"><div><h2>إدارة المحافظ</h2><small>محافظ USDT، المحافظ البنكية والإلكترونية، إنشاء محفظة جديدة، تعطيل أو حذف، ومراقبة الأرصدة</small></div></div>
            <form class="form-grid" method="post" action="{{ route('control.wallets.store', [], false) }}">
                ...
                <button class="btn" type="submit">إنشاء / تحديث محفظة</button>
            </form>
            <div class="table-wrap">
                <table>
        ...
        <section class="two-col" id="currencies">
            <div class="panel">
                <div class="panel-head"><div><h2>إدارة العملات والشبكات</h2><small>العملات المدعومة، التفعيل، رسوم الشبكة، وحدود التحويل المستقبلية</small></div></div>
            ...
            <div class="panel">
                <div class="panel-head"><div><h2>إدارة الرسوم</h2><small>رسوم ثابتة، رسوم نسبية، ورسوم حسب نوع التحويل</small></div></div>
                <div class="table-wrap">
                    <table>
        ...
        <section class="section-grid" id="orders">
            <div class="module"><h3>إدارة الطلبات</h3><small>ملخص الطلبات حسب نوع العملية</small><ul><li><span>الطلبات الجديدة</span><b>{{ number_format($statusCounts['pending'] ?? 0) }}</b></li><li><span>طلبات الإيداع</span><b>{{ number_format($typeCounts['deposit'] ?? 0) }}</b></li><li><span>طلبات السحب</span><b>{{ number_format($typeCounts['withdraw'] ?? 0) }}</b></li><li><span>طلبات الدعم</span><b>قيد الربط</b></li></ul></div>
            <div class="module" id="reports"><h3>التقارير</h3><small>أرباح، حجم تداول، عملات نشطة، وعملاء نشطون</small><ul><li><span>الأرباح</span><b>{{ number_format((float) $stats['profits'], 2) }}</b></li><li><span>حجم التداول</span><b>{{ number_format((float) $dailyMovement->sum('amount'), 2) }}</b></li><li><span>تصدير Excel</span><b>قيد الربط</b></li><li><span>تصدير PDF</span><b>قيد الربط</b></li></ul></div>
            <div class="module"><h3>ميزات احترافية</h3><small>مؤشرات أداء ومراقبة وموافقات</small><ul><li><span>KPIs لحظية</span><b>مفعلة</b></li><li><span>مراقبة TRC20/ERC20</span><b>تحتاج مزود شبكة</b></li><li><span>تنبيه العمليات المشبوهة</span><b>قيد الربط</b></li><li><span>موافقات متعددة المستويات</span><b>قيد الربط</b></li></ul></div>
        </section>
        ...
        <section class="panel" id="audit">
            <div class="panel-head"><div><h2>سجل العمليات</h2><small>من قام بالتعديل، وقت العملية، IP، الجهاز، وسجل تغييرات الإدارة</small></div></div>
            <div class="table-wrap">
                <table>
        ...
        <section class="section-grid">
            <div class="module" id="roles"><h3>الصلاحيات</h3><small>Admin، Support، Auditor وصلاحيات مخصصة</small><ul><li><span>Admin</span><b>كامل</b></li><li><span>Support</span><b>قيد الربط</b></li><li><span>Auditor</span><b>قيد الربط</b></li><li><span>صلاحيات مخصصة</span><b>قيد الربط</b></li></ul></div>
            <div class="module" id="security"><h3>الأمان</h3><small>2FA، الجلسات، الأجهزة الموثوقة، القفل ومحاولات الدخول</small><ul><li><span>المصادقة الثنائية</span><b>قيد الربط</b></li><li><span>إدارة الجلسات</span><b>Laravel Sessions</b></li><li><span>قفل الحساب</span><b>متاح من المستخدمين</b></li><li><span>سجل تسجيل الدخول</span><b>قيد الربط</b></li></ul></div>
            <div class="module" id="settings"><h3>الإعدادات</h3><small>الشركة، الشعار، اللغة، المنطقة الزمنية، API، البريد والرسائل</small><ul><li><span>معلومات الشركة</span><b>USDT STORE</b></li><li><span>اللغة</span><b>العربية</b></li><li><span>المنطقة الزمنية</span><b>{{ config('app.timezone') }}</b></li><li><span>مفاتيح API والبريد</span><b>ملفات env</b></li></ul></div>
        </section>

        <section class="section-grid" id="support">
            <div class="module"><h3>الدعم الفني</h3><small>تذاكر، محادثة، ومتابعة حالة الطلبات</small><ul><li><span>نظام تذاكر</span><b>قيد الربط</b></li><li><span>محادثة العملاء</span><b>قيد الربط</b></li><li><span>متابعة الطلبات</span><b>من التحويلات</b></li></ul></div>
            <div class="module"><h3>النسخ الاحتياطي</h3><small>استعادة وتشغيل آمن</small><ul><li><span>نسخ قاعدة البيانات</span><b>قيد الربط</b></li><li><span>استعادة</span><b>قيد الربط</b></li><li><span>مراقبة الصحة</span><b>/api/health</b></li></ul></div>
            <div class="module"><h3>تنبيهات المخاطر</h3><small>كشف العمليات الكبيرة أو المشبوهة</small><ul><li><span>مبالغ كبيرة</span><b>قيد الربط</b></li><li><span>تكرار سريع</span><b>قيد الربط</b></li><li><span>محافظ خطرة</span><b>تحتاج مزود خارجي</b></li></ul></div>
        </section>
    </main>
</div>
</body>
